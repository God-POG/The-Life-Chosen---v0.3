...
const APP_SHELL_URLS = [APP_ROOT_URL.href, INDEX_URL.href];
...
self.addEventListener("install", (event) => {
  ...
  event.waitUntil((async () => {
    ...
    await Promise.all(
      APP_SHELL_URLS.map((href) =>
        cache.add(new Request(href, { cache: "reload" })).catch(() => null)
      )
    );
  })());
});
...
async function networkFirst(request) {
  ...
  try {
    ...
    if (isHtmlNavigation(request)) {
      const fallback =
        await cache.match(APP_ROOT_URL.href) ||
        await cache.match(INDEX_URL.href) ||
